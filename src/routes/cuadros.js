...
routerCuadro.put("/:id", async (req, res) =>{
  try {
    ...
    await pool.query("UPDATE cuadros SET nombre = $1, estudio = $2, sinopsis = $3, link_anime = $4, best_waifu = $5, codigo_obra = $6, imagen = $7", [nombre, estudio, sinopsis, link_anime, best_waifu, codigo_obra, imagen])
    res.status(201).json({message: "el cuadro se ha actualizado correctamente"})
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
})
...
routerCuadro.delete("/:id", async (req, res) => {
  try {   
    ...
    res.status(200).json({message:"el cuadro ha sido eliminado corectamente"})
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
})